...
let path_length adj_matrix path =
  let s = ref 0 in
  for i = 0 to Array.length path - 2 do
    s := !s + adj_matrix.(path.(i)).(path.(i + 1))
  done;
  !s + adj_matrix.(path.(0)).(path.(Array.length path - 1))
...
let print_path ?(oc = stdout) path =
  Array.iteri
    (fun i v ->
      ...
      else if i = Array.length path - 1 then Printf.fprintf oc "-> [%d]\n"
...
let print_error_ratio ?(oc = stdout) ?(file_path = "tsp_instances") path eval
    city_config =
