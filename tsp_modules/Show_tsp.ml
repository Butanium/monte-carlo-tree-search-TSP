...
let show_cities cities =
    ...
    set_line_width 1;
    let update_color i =
        let r = int_of_float @@ (fl @@ city_count - i - 1) /. fl (city_count - 1) *. 255. in 
    ...
    Array.iteri (fun i (x,y) ->  update_color i; fill_circle x y params.city_size;) @@ Array.map (coordToScreen (maxX, maxY)) cities
...
let show_cities_names cities = 
    ...
    set_color white;
    let maxX, maxY = Array.fold_left (fun (maxX, maxY) (x,y) -> (max maxX x), (max maxY y)) (0.,0.) cities in
    Array.iteri (fun i (x,y) -> let xs, ys = text_size @@ string_of_int i in 
        moveto (x-params.city_size*0 - xs/2) (y-params.city_size/2*0 - ys/2); draw_string @@ string_of_int i) @@ 
    Array.map (coordToScreen (maxX, maxY)) cities
let show_solution ?(title = "Path") cities sol =
    let sorted_cities = Array.init (Array.length sol) 
        (fun i -> cities.(sol.(i))) in 
