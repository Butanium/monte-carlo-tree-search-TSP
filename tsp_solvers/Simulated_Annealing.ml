...
type argument = {
  ...
  mutable city_count : int;
  mutable solution : int array;
  mutable eval : int -> int -> int;
  mutable i : int;
  mutable j : int;
}
...
let random_choice () =
  let rnd shift =
    int_of_float @@ (Random.float arg.problem_size +. 0.5 -. shift)
  in
  let i = rnd 0. in
  let j = rnd 1. in
...
let get_delta_fun mode =
  let m = function
    ...
    | x -> x
  in
  let edge_len x y = arg.eval arg.solution.(x) arg.solution.(y) in
  fun () ->
    let i, j = (arg.i, arg.j) in
    match mode with
    | Swap ->
        edge_len (m @@ (i - 1)) j
        + edge_len (m @@ (i + 1)) j
  ...
  let rec aux temp =
    if temp <= final_T then arg.solution
      ...
      for _ = 1 to iter_count do
        random_choice ();
        if Random.float 1. < exp (-.float_of_int (delta ()) /. temp) then
          randomize ()
      done;
