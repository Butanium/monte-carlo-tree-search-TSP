...
module MCTS = struct
    ...
    and heritage =  {used_cities: IntSet.t; init_score: int; init_length: int; start_path: int list}
    ...
    type select_mode = Ucb1 of float | Ucb1_mst of float | Ucb1_average of float
    ...
    let get_node_score_fun average_mode eval city_count =
        ...
        | Ucb1_average _ -> fun _ -> failwith "ecart type not defined"
    ...
    let rec select node arg =
        ...
        match RndQ.is_empty node.base.poss_cities, node.base.sons with
            | true, [] -> (try (retropropagation node (node.base.accDist + arg.eval node.base.city arg.start) arg)
    ...
    let mcts ?(debug=false) city_count eval rnd_creation_mode opt_mode select_mode average_mode min_conv min_playout max_playout max_time =
        ...
        while (getRoot()).init_length+1 < city_count do
            ...
            while playout_count() < max_playout && (playout_count() < min_playout || getRatio() < min_conv) && Sys.time() -. t < max_time do
                ...
                | true, Ucb1_average e -> let moy = List.fold_left (fun acc n -> acc +. n.base.accScore /. n.base.visit) 0. (!root).base.sons
